...
def render_login() -> None:
    st.caption("Versão 1: funcionário só escolhe o produto, confirma e imprime a etiqueta.")
...
def render_label_actions(launch: dict[str, Any]) -> None:
    ...
    if st.button("Imprimir agora na térmica", key=f"print_now_{launch['id']}", type="primary", width='stretch'):
        ok, message = send_to_thermal_printer(tspl_label)
        if ok:
            st.session_state.last_label_launch_id = None
            st.success(message)
            st.rerun()
    ...
    st.caption(f"Formato térmico: TSPL 60x30 mm, GAP 2 mm, fila {PRINTER_QUEUE}.")
    ...
    for product in filtered:
        if st.button(
            ...
            if launch is not None:
                ok, message = send_to_thermal_printer(build_tspl_label(launch))
                if ok:
                    st.toast(f"{product['name']} impresso.")
                else:
                    st.error(message)
            st.rerun()
